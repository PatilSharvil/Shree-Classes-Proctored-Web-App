...
const createExam = async (req, res) => {
  try {
    ...
    if (!title) return errorResponse(res, 400, 'Exam title is required.');
    if (!duration_minutes) return errorResponse(res, 400, 'Duration is required.');
    if (Number(duration_minutes) <= 0) return errorResponse(res, 400, 'Duration must be a positive number (minimum 1 minute).');
    if (Number(duration_minutes) > 1440) return errorResponse(res, 400, 'Duration cannot exceed 24 hours (1440 minutes).');
    if (!total_marks) return errorResponse(res, 400, 'Total marks is required.');
    if (Number(total_marks) <= 0) return errorResponse(res, 400, 'Total marks must be a positive number.');
    if (negative_marks !== undefined && Number(negative_marks) < 0) return errorResponse(res, 400, 'Negative marks cannot be negative.');
    if (passing_percentage !== undefined && (Number(passing_percentage) < 0 || Number(passing_percentage) > 100)) return errorResponse(res, 400, 'Passing percentage must be between 0 and 100.');
    if (tab_switch_threshold !== undefined && (Number(tab_switch_threshold) < 1 || Number(tab_switch_threshold) > 100)) return errorResponse(res, 400, 'Tab switch threshold must be between 1 and 100.');
    if (looking_away_threshold !== undefined && (Number(looking_away_threshold) < 1 || Number(looking_away_threshold) > 100)) return errorResponse(res, 400, 'Looking away threshold must be between 1 and 100.');
...
const getExamById = async (req, res) => {
  try {
    ...
    if (error.message === 'Exam not found.') return errorResponse(res, 404, error.message);
...
const updateExam = async (req, res) => {
  try {
    ...
    if (error.message === 'Exam not found.') return errorResponse(res, 404, error.message);
    if (error.message === 'No fields to update.') return errorResponse(res, 400, error.message);
...
const checkAvailability = async (req, res) => {
  try {
    const result = await examService.isExamAvailable(req.params.id);
